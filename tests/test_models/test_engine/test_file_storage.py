...
class TestFileStorage(unittest.TestCase):
    ...
    @unittest.skipIf(models.storage_t == 'db', "not testing file storage")
    def test_new(self):
        """test that new adds an objc to the FileStorage.__objects attr"""
        storage = FileStorage()
    ...
    @unittest.skipIf(models.storage_t == 'db', "not testing file storage")
    def test_save(self):
        """Test that save properly saves to file.json"""
        storage = FileStorage()
        new_dict = {}
        for key, value in classes.items():
            instance = value()
    ...
    @unittest.skipIf(models.storage_t == 'db', "not testing file storage")
    def test_get(self):
        """checks method for obtaining an instance file storage"""
        storage = FileStorage()
        dic = {"name": "Vecindad"}
        instance = State(**dic)
        storage.new(instance)
    ...
    @unittest.skipIf(models.storage_t == 'db', "not testing file storage")
    def test_count(self):
        """checks the count method file storage"""
        storage = FileStorage()
        dic = {"name": "Vecindad"}
        state = State(**dic)
        storage.new(state)
