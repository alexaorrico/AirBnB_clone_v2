...
class TestFileStorage(unittest.TestCase):
    ...
    @unittest.skipIf(models.storage_t == "db", "not testing file storage")
    def test_get(self):
        """Test that get properly retrives an object from file.json"""
        storage = FileStorage()
    ...
    def test_count(self):
        """
        Test that count properly returns
        numbers the objects saved to file.json
        """
        storage = FileStorage()
        ...
        for _ in range(3):
            new_instance = cls_choice()
            inst_id = cls_choice.__name__ + "." + new_instance.id
            test_dict[inst_id] = new_instance

        FileStorage._FileStorage__objects = test_dict
        self.assertEqual(storage.count(cls_choice), 3)
