...
def get_post_cities(state_id):
    """Handles GET (retrieve all cities in a state) and POST (create new city in a state) requests"""
...
def get_put_delete_city(city_id):
    """Handles GET (retrieve), PUT (update), and DELETE (remove) requests for a specific city"""
    ...
    if city is None:
        ...
        # Update attributes of the city based on PUT data in JSON format
        ...
        storage.save()  # Save changes
        return jsonify(city.to_dict()), 200  # Return updated city details
