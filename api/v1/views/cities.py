...
def cities_by_state(state_id):
    """return city objects by state with id:state_id"""
...
def show_city(city_id):
    """return a City object identified with id:city_id"""
...
def insert_city(state_id):
    """creates a City object to the designated State"""
    state = storage.get(State, state_id)
    if state is None:
        abort(404)
...
def update_city(city_id):
    """update a City object identified by id=city_id"""
    city = storage.get(City, city_id)
    if city is None:
        abort(404)
    props = request.get_json()
