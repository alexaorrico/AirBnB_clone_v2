...
def update_place(place_id):
    """ Makes updates on a given place """
    if not request.get_json():
        return make_response(jsonify({"error": "Not a JSON"}), 400)
...
def search_places():
    ...
    if fp and len(fp):
        states = fp.get('states', None)
        cities = fp.get('cities', None)
        amenities = fp.get('amenities', None)

    if not fp or not len(fp) or (
            not states and
            not cities and
            not amenities):
